...
- Assets: `libXDL_wgpu.a`, `SDL3.a`, `SDL3_image.a`, `SDL3_mixer.a`, `SDL3_ttf.a` and `THIRD-PARTY-NOTICES.txt`. `SDL3_ttf.a` also contains the libraries SDL_ttf vendors (FreeType, HarfBuzz, plutosvg and plutovg): SDL_ttf's static build puts them into its archive, so it links on its own. GitHub shows the SHA-256 of each asset.
