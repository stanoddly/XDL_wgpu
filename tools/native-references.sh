...
# Usage: tools/native-references.sh [--url | --file <dir>] [tag]
#   --url         NativeUrlReference items with the download URL and SHA-256 of each archive (the default)
#   --file <dir>  downloads the archives into <dir>, checks their SHA-256, and prints NativeFileReference items with <dir> as given
...
while [ $# -gt 0 ]; do
    case $1 in
        --url) mode=url; shift ;;
        --file) [ $# -ge 2 ] || usage 2; mode=file; dir=${2%/}; shift 2 ;;
        -h | --help) usage 0 ;;
        -*) usage 2 ;;
